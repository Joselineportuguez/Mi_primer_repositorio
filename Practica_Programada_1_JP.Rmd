...
```{r}
datos_netflix_info <- datos_netflix_info  |>
  mutate(across(c(Pais), function(x) ifelse(x == "	
united states", "United States", x))) |>
  mutate(across(c(Pais), function(x) ifelse(x == "india", "India", x))) |>
  mutate(across(c(Pais), function(x) ifelse(x == "pakistan", "Pakistan", x)))
```
